...
class XLSR53_CPU(IRecognitionModel):
    ...
    def __init__(self, path: Path, sample_rate: int) -> None:
        ...
        self.__model = Wav2Vec2ForCTC.from_pretrained(self.__path)
        self.__processor = Wav2Vec2Processor.from_pretrained(self.__path)
...
class XLSR53_GPU(IRecognitionModel):
    __necessary_files: list[str] = [
        'config.json',
        'preprocessor_config.json',
        'pytorch_model.bin',
    ...
    def __init__(self, path: Path, sample_rate: int) -> None:
        ...
        self.__model = Wav2Vec2ForCTC.from_pretrained(self.__path).to('cuda')
        self.__processor = Wav2Vec2Processor.from_pretrained(self.__path)
        self.__sample_rate = sample_rate
